...
**Current:** declared `requires` and `exports` contracts plus an in-memory event path. Production storage is JSON `ModuleData`. On disposable databases only, compiled `mod_<moduleId>` tables may hold a development shadow copy for backfill and dual-read validation.
