...
Each Module declares the `group` and optional `subgroup` its pages belong to, and the sidebar registry places them. `@86d-store/orders` lands under **Sales → Orders**. `@86d-store/blog` lands under **Marketing → Publishing**. There is no navigation config file for you to maintain, and no way for two Modules to fight over a slot.
