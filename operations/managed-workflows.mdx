...
The managed `*.86d.store` front door attaches the public FQDN to Railway as a custom domain, then publishes Railway's routing CNAME **and** VERIFY TXT into Cloudflare (DNS-only). Compensation and development teardown remove both owned DNS records before detaching the Railway domain or deleting the Railway project.
