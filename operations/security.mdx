...
- grant cross-Module reads only through column-projected published views (target); production storage is still JSON `ModuleData`
...
The Store Runtime uses Prisma and PostgreSQL. Generated queries are parameterized, which handles the common injection paths. Raw-query call sites still need reading by a person.
