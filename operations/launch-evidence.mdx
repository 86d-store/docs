...
Pass two is the live Console rehearsal. The final reset follows it, and pass three recreates the Store through the agent flow. Genuine activity and marketed launch wait for both passes and Finix readiness.
...
The register is append-only and access-controlled, because entries reference provider artifacts and production identifiers. What is published is the status, not the underlying record.
...
- passes two and three complete in reset order, with Finix card, Apple Pay, Google Pay, fee, refund, verified-event, and settlement/reconciliation evidence
