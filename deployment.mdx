...
86d.store ships with a `Dockerfile` and a `docker-compose.yml`, and it runs on Railway or Vercel without modification. Pick the tab that matches where you want it. All three paths are you hosting a [Store Runtime](/concepts/architecture) yourself, which means you also own the database, the backups, and the uptime. Those host names appear here only because you chose them.

A deployment created through [86d.app](/introduction) is a different arrangement: 86d provisions the machines, runs the deployment, and bills you as a [Managed Deployment](/resources/glossary#86d-cloud). That path never names the suppliers behind managed hosting, DNS, or payments. You operate 86d Cloud. See [86d Cloud plans](/resources/cloud-plans) and [architecture](/concepts/architecture#what-you-see-from-86d).
