...
A Store waiting on payment activation does not have to turn Shoppers away. It can take a **reservation**: a purchase intent with the charge deferred, disclosed as such at Checkout and again in the confirmation email. The Shopper can cancel, the deadline is fixed, and only card is offered because wallets need an approved merchant account.

When payments activate, the reservation captures and becomes an Order. If activation is declined or the deadline passes, it cancels, the Shopper is notified, and the stored payment method is deleted.

That is different from a [Checkout Request](/resources/glossary#checkout-request), which is what a Store saves when tax or a Shipping quote is the missing piece rather than payment. A Checkout Request stores no payment details, reserves no stock, and creates no Order.
